...
```{r plot-alpha-div-fxn}
PlotAlphaDiv <- function(df, index, yLabel, y_min, y_max) {
  set.seed(2908795)
  ggplot(data = df, aes_string(x = "Growth_Status_Prior", y = index)) +
    ...
    ylim(y_min, y_max) +
    stat_compare_means(label = "p.format", method = "kruskal.test",
                       hide.ns = TRUE, label.x.npc = 0, label.y.npc = 0.9,
                       size = 3.5) +
    theme(axis.title.x = element_blank(),
          axis.text.x = element_text(angle = 45, hjust = 1),
          axis.title.y = element_text(size = 10))
...
```{r figure2pC-bacterial-richness - unpaired}
#----- unpaired analysis -----#
plot2_C_unpaired <- map(.x = list("T1" = alphaDivT1, "T2" = alphaDivT2),
                        .f = PlotAlphaDiv,
                        index = "Observed", yLabel = NULL,
                        y_min = 50, y_max = 350)


plot2_C_unpaired_arranged <- annotate_figure(ggarrange(plot2_C_unpaired$T1,
                                                       plot2_C_unpaired$T2,
                                                       ncol = 2),
                                             left = text_grob("Richness", rot = 90))
plot2_C_unpaired_arranged
```
...
```{r figure2pD-bacterial-shannon-diversity-paired}
#----- paired analysis -----#
...
```{r figure2pD-bacterial-diversity - unpaired}
#----- unpaired analysis -----#
plot2_D_unpaired <- map(.x = list("T1" = alphaDivT1, "T2" = alphaDivT2),
                        .f = PlotAlphaDiv,
                        index = "Shannon", yLabel = NULL,
                        y_min = 2, y_max = 5)


plot2_D_unpaired_arranged <- annotate_figure(ggarrange(plot2_D_unpaired$T1,
                                                       plot2_D_unpaired$T2,
                                                       ncol = 2),
                                             left = text_grob("Shannon Diversity",
                                                              rot = 90))
plot2_D_unpaired_arranged
```
...
```{r figure2pE-bacterial-pcoa-wUniFrac-paired}
#---- paired analysis -----#
...
```{r figure2-panel-arrange-paired, fig.width=11, fig.height=8.5}
...
```{r figure2-panel-arrange-unpaired, fig.width=11, fig.height=8.5}
fig2UnpairedTopRow <- ggarrange(plot2_A, plot2_B, labels = c("A", "B"), ncol = 2)

fig2UnpairedMiddleRow <- ggarrange(plot2_C_unpaired_arranged, 
                                   plot2_D_unpaired_arranged,
                                   ncol = 2,
                                   labels = c("C", "D"))
