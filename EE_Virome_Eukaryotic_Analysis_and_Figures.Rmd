...
sampleNamesLUT <- select(eukaryoticSampleData,
                         unique_id, sample_long_name)
...
anelloGroundTruth <-  ggplot(eukFamilyNormalizedAbdDFList$Anelloviridae, 
                            aes_string(x = "Growth_Status_Prior", y = "Abundance")) +
    geom_boxplot(outlier.shape = NA) +
    geom_jitter(alpha = 0.7, width = 0.15, size = 1.5) +
    scale_y_log10() +
    ylab("Normalized Abd. (log10)") +
    ggtitle("Anelloviridae") +
    stat_compare_means(method = "kruskal.test", 
                       label = "p.format",
                       size = 3.5,
                       label.x.npc = 0.5) +
